...
Notes (2026-08-19): **the gofmt gate was itself inverted and had never worked.** `lint.yml` ran `! gofmt -d -l . | awk '{print} END{exit NR>0}'`; `!` negates the exit status of the whole pipeline, not just `gofmt`, so the step **failed on a correctly formatted tree and passed on an unformatted one** — gofmt was never actually enforced. `make check` had it right (no `!`); CI had drifted, which is exactly the "CI and `make check` have diverged" item above. Fixed in `5ea1330` (PR #13). It surfaced only when a PR was opened against a formatted tree.
Add to this WU's scope: **each gate must self-test.** `check-scope.sh` already proves itself against committed pass/fail fixtures on every run (`scripts/testdata/check-scope/`) — the gofmt, sqlc-diff, templ-diff and `go.mod`-tidy gates have no equivalent, so an inverted or no-op gate passes silently. Give each one a deliberately-violating fixture that must fail it.
